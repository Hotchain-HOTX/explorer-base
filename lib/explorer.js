...
module.exports = {
  ...
  prepare_vout: function(vout, txid, vin, cb) {
	...
	var addressParsed = 'zerocoin';
	if(vout[i].scriptPubKey.addresses){
	addressParsed = vout[i].scriptPubKey.addresses[0]
	}
        module.exports.is_unique(arr_vout, addressParsed, function(unique, index) {
          if (unique == true) {
            // unique vout
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({addresses: addressParsed, amount: amount_sat});
              loop.next();
            });
          } else {
            // already exists
    ...
    if (input.coinbase) {
      ...
      module.exports.get_rawtransaction(input.txid, function(tx){
        if (tx) {
          ...
          if(tx.vout)
            l = tx.vout.length;
          module.exports.syncLoop(l, function (loop) {
            var i = loop.iteration();
